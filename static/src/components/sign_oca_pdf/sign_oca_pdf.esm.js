...
export default class SignOcaPdf extends SignOcaPdfCommon {
    ...
    postIframeField(item) {
        if (item.role_id === this.info.role_id) {
            ...
            // Every field assigned to the active signer is mandatory.  Keep
            // the visual cue independent from the legacy template checkbox.
            signatureItem[0].classList.add("sign_oca_field_required");
    ...
    checkFilledAll() {
        const signerItems = Object.values(this.info.items).filter(
            (item) => item.role_id === this.info.role_id
        );
        this.to_sign_update = signerItems.every((item) => {
            const field = SignRegistry.get(item.field_type);
            return field && field.check(item);
        });
        this.checkToSign();
    }
}
